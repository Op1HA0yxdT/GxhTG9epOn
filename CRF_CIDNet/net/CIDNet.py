...
class CSE_RLCA(nn.Module):
    def __init__(self, dim,num_heads, bias=False):
        super(CSE_LCA, self).__init__()
        self.hv_lca = HV_RLCA(dim, num_heads, bias)
        self.i_lca = I_RLCA(dim, num_heads, bias)

    def forward(self, hv, i, residual):
        hv_out = self.hv_lca(hv, i, residual)
        i_out = self.i_lca(i, hv, residual)
        return hv_out, i_out
