...
try:
    from CSE_arch import Transformer, TransformerU
except:
    from .CSE_arch import Transformer, TransformerU

# used as wrapper class
class RetinexFormer(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        ...
        # ========= CRF-Baseline
        self.body = Transformer(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, num_blocks=num_blocks)
        
        # ========= Baseline
        # self.body = TransformerU(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, num_blocks=num_blocks)
...
if __name__ == '__main__':
    ...
    from CSE_arch import TransformerBlock
    # model = RetinexFormer(stage=1, n_feat=32, num_blocks=[1,2,2]).cuda()
    model = TransformerBlock(in_channels=64, num_heads=2).cuda()
    print(model)

    # Compute FLOPs and Params
    
    inputs = torch.randn((1, 64, 256, 256)).cuda()
    inputs = (inputs, inputs)
    flops = FlopCountAnalysis(model, inputs)
    n_param = sum([p.nelement() for p in model.parameters()])  # Count parameters
    print(f'GMac: {flops.total() / (1024 * 1024 * 1024)}')
    print(f'Params: {n_param}')

    # Run inference with torch.no_grad() to prevent OOM
    # with torch.inference_mode():
    #     inputs = torch.randn((1, 3, 2000, 3000)).cuda()
    #     model(inputs)
