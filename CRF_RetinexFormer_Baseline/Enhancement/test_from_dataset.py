    ...
    with torch.inference_mode():
        for inp_path, tar_path in tqdm(zip(input_paths, target_paths), total=len(target_paths)):
            ...
            if h < 2200 and w < 3050:
                ...
                # Four-way split and process
                input_1 = input_[:, :, :, 0::4]  # Columns at indices 0, 4, 8, ...
                input_2 = input_[:, :, :, 1::4]  # Columns at indices 1, 5, 9, ...
                input_3 = input_[:, :, :, 2::4]  # Columns at indices 2, 6, 10, ...
                input_4 = input_[:, :, :, 3::4]  # Columns at indices 3, 7, 11, ...
                ...
                # Reconstruct the output by interleaving columns
            ...
            # Function to save images with compression
            # def save_compressed_img(filepath, img, format="png", bit_depth=6, quantize=False):
            #     """ Saves an image with reduced bit depth for compression. 
            #         - Converts to 8-bit uint8 if needed.
            #         - Applies optional color quantization (posterization).
            #     """
            #     img = img_as_ubyte(img)  # Convert float images to 8-bit
            #     img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)  # Convert RGB to BGR

            #     # Reduce bit depth (Posterization)
            #     if quantize:
            #         if bit_depth == 7:
            #             img = (img // 2) * 2  # 7-bit color depth (128 levels)
            #         elif bit_depth == 6:
            #             img = (img // 4) * 4  # 6-bit color depth (64 levels)
            #         elif bit_depth == 5:
            #             img = (img // 8) * 8  # 5-bit color depth (32 levels)
            #         elif bit_depth == 4:
            #             img = (img // 16) * 16  # 4-bit color depth (16 levels)

            #     if format == "png":
            #         # Save PNG with high compression
            #         cv2.imwrite(filepath, img, [cv2.IMWRITE_PNG_COMPRESSION, 9])
            #     elif format == "jpg" or format == "jpeg":
            #         # Save JPEG with adjustable quality
            ...
            # # Inside the main loop, replace utils.save_img with save_compressed_img:
            # if output_dir != '':
            #     save_compressed_img(os.path.join(output_dir, os.path.splitext(
            #         os.path.split(inp_path)[-1])[0] + '.png'), restored)
            # else:
            #     save_compressed_img(os.path.join(result_dir, os.path.splitext(
                    # os.path.split(inp_path)[-1])[0] + '.png'), restored)
